...
class Vrrefund extends Model {
     ...
     * @return type
     */
    public function getVrrefundList($condition = array(), $page = '', $limit = '', $fields = '*') {
        if($page){
            $result = db('vrrefund')->field($fields)->where($condition)->order('refund_id desc')->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $result;
            $result = $result->items();
        }else{
            $result = db('vrrefund')->field($fields)->where($condition)->order('refund_id desc')->select();
        }
